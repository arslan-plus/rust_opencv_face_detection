...
fn get_faces(frame: &opencv::prelude::Mat) -> opencv::Result<types::VectorOfRect> {
    ...
    face.detect_multi_scale(&gray, &mut faces, 1.1, 3, 0, core::Size{width: 300, height: 300 }, core::Size { width: 0, height: 0 })?;

    return Ok(faces);
}

fn run() -> opencv::Result<()> { 
    let mut frame = opencv::imgcodecs::imread("./data/input.jpg", opencv::imgcodecs::IMREAD_COLOR)?;
    ...
    opencv::imgcodecs::imwrite("./data/output.jpg", &frame, &mut params)?;
